...
import { NARRATION_TITLE, NARRATION_BODY, CORRECT_ANSWERS, CORRECT_POINTS, INCORRECT_POINTS } from '../data/quiz/paulCartoon';
...
// Follows Amber2: free-text "who said it" beat. On a correct answer the
// cartoon background glitches/dissolves into the real photo of Paul (same
// reveal beat as ButlinsDad/Scene1) before showing the continue button.
export class PaulCartoon extends Scene
{
    ...
    create ()
    {
        ...
        this.background = this.fitToCanvas(this.add.image(CENTER_X, CENTER_Y, 'paul-cartoon'));
    ...
    {
        const cartoonImage = this.background;

        const realImage = this.fitToCanvas(this.add.image(CENTER_X, CENTER_Y, 'paul'))
            .setAlpha(0)
            .setDepth(-1);
    ...
    private showContinue (): void
    {
        ...
        const button = new Button(this, {
            ...
            label: 'CONTINUE',
            confirmOnEnter: true,
            onClick: () => this.scene.start('CousinRace')
        });
